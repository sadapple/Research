...
plot(rep(0,nrow(beta.mat)),beta.mat[,1]^(1),pch=21,cex=.5,xlim=c(0,length(lamda.vec)-1),ylim =c(-y.med,y.med),xlab="kinks",ylab="Coefficients",main="Regularization Path, p=4")
for (i in 1:nrow(beta.mat)){
  lines(c(0:(length(lamda.vec)-1)),beta.mat[i,]^(1))
}

for (i in 1:nrow(beta.mat)){
  points(c(0:(length(lamda.vec)-1)),beta.mat[i,]^(1),pch=21,cex=.5,col="red")
}
